...
var question3 = new Question(
  `Do the children go to school every day? ______ .`,
  3,
  "Yes,they do"
);
...
var question4 = new Question(
  `They always go to school ______ bicycle.`,
  4,
  "by"
);
...
function addAnswer(ul, m) {
  for (let i = 0; i < questions[m].answers.length; i++) {
    ...
    label.setAttribute(
      "for",
      `answer${questions[m].questionId}-${questions[m].answers[i].id}`
    );
...
function showQuestion() {
  ...
  var markedQuestionBtn = document.getElementById(
    `ques${currentQuestionIndex}Index`
  );
...
function calcResult() {
  ...
  for (let ans = 0; ans < questions.length; ans++) {
    ...
    var choosenAnswer = document.querySelector(
      `input[name="ques${questionId}"]:checked`
    );
